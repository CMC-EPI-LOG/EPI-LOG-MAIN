...
test('프로필 변경 중에는 전체 데이터 컴포넌트가 스켈레톤으로 전환된다', async ({ page }) => {
  ...
  await expect(page.getByText('KF80 마스크 챙기기')).toHaveCount(0);
  await expect(page.getByText('실외 활동은 30분 내로 조절하기')).toHaveCount(0);
...
test('첫 화면은 요약 + 액션 우선으로 보이고 상세 섹션은 접혀 있다', async ({ page }) => {
