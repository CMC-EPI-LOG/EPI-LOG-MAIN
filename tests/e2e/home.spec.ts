...
async function waitForHydratedDashboard(page: Page) {
  ...
  await page.waitForFunction(() => {
    return (
      window &&
      typeof (window as Window & { __AISOOM_TEST_HOOKS_READY__?: boolean }).__AISOOM_TEST_HOOKS_READY__ === 'boolean' &&
      (window as Window & { __AISOOM_TEST_HOOKS_READY__?: boolean }).__AISOOM_TEST_HOOKS_READY__ === true
    );
  });
}
...
test('reverse-geocode fallback 응답이어도 지역 제한 문구 없이 기본 관측소로 이어진다', async ({ page }) => {
  await page.unroute('**/api/reverse-geocode');
  await page.route('**/api/reverse-geocode', async (route) => {
    await route.fulfill({
      status: 200,
      contentType: 'application/json',
  ...
  await expect(page.getByText("현재 위치를 정확히 확인하지 못해 '중구' 기준으로 보여드려요 🏢")).toBeVisible();
  await expect(page.getByText('지원 지역 밖')).toHaveCount(0);
  await expect(page.getByTestId('hero-outing-status')).toContainText('외출 비권장');
});
...
test('초기 진입 시 기본 fallback 문구 대신 스켈레톤을 먼저 보여주고 핵심 데이터부터 순차 노출한다', async ({ page }) => {
  await page.unroute('**/api/daily-report');
  await page.unroute('**/api/clothing-recommendation');

  let clothingRequests = 0;
  ...
  await expect(page.getByText('AI 선생님이 잠시 쉬고 있어요.')).toHaveCount(0);
  await expect(page.getByText('아직 추천 액션을 준비 중이에요.')).toHaveCount(0);
...
test('위치 모달을 열고 닫을 수 있다', async ({ page }) => {
  ...
  await page.evaluate(() => {
    window.dispatchEvent(
      new CustomEvent('aisoom:test-location-modal', {
        detail: { open: true },
      }),
    );
  });
...
test('첫 화면은 요약 + 액션 + 옷차림 버튼 우선으로 보이고 상세 섹션은 접혀 있다', async ({ page }) => {
  ...
  await expect(page.getByText('오늘은 실외 활동 가능해요')).toBeVisible();
  await expect(page.getByText('아이를 위한 오늘의 액션')).toBeVisible();
  await expect(page.getByTestId('hero-clothing-open')).toBeVisible();
  await expect(page.getByTestId('hero-clothing-open')).toContainText(/옷차림\s*\(현재날씨/);
...
test('주소/프로필 변경 시 변경 데이터와 프로필 스켈레톤 캡션이 반영된다', async ({ page }) => {
...
test('daily-report 실패 시 마지막 성공 snapshot으로 복구 배너를 노출한다', async ({ page }) => {
  const recentSavedAt = new Date(Date.now() - 10 * 60 * 1000).toISOString();

  await page.addInitScript((savedAt) => {
    window.localStorage.setItem(
      'aisoom:last-report-snapshot',
      JSON.stringify({
        savedAt,
        stationName: '중구',
        data: {
          profileSignature: 'unknown::none::none::',
          report: {
            airQuality: {
              stationName: '중구',
              grade: 'NORMAL',
            ...
            aiGuide: {
              summary: '저장된 안내예요',
              detail: '마지막 성공 데이터로 복구했어요.',
              threeReason: ['이전 성공 응답', '네트워크 오류 복구', '잠시 후 다시 시도'],
              detailAnswer: '마지막 성공 데이터로 복구했어요.',
              actionItems: ['실외 활동은 짧게'],
              activityRecommendation: '짧은 외출 가능',
              maskRecommendation: '민감군은 마스크 권장',
              references: [],
            },
            reliability: {
              status: 'LIVE',
              label: '최근 1시간 기준 실측 데이터',
              description: '이전 저장본입니다.',
              requestedStation: '중구',
              resolvedStation: '중구',
              triedStations: ['중구'],
              updatedAt: savedAt,
  ...
  await expect(page.getByTestId('stale-snapshot-banner')).toContainText('마지막 성공 데이터를 보여주고 있어요');
  await expect(page.getByText('저장된 안내예요')).toBeVisible();
});
